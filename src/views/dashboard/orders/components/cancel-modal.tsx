...
export function CancelModal({ open, onClose, order, onSuccess }: Props) {
   const [reason, setReason] = useState('');
   const [images, setImages] = useState<(File | string)[]>([]);
   const [loading, setLoading] = useState(false);

   const { adminCancel } = useOrderStore();
   ...
   const handleSubmit = async () => {
      ...
      try {
         ...
         const response = await adminCancel({ id: order.id!, data: formData });
         ...
         <DialogTitle>
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
               <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  <Iconify icon="solar:close-circle-bold" width={28} color="error.main" />
